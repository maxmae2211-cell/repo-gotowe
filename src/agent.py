...
ALLOWED_MODES = {"standard", "jmeter-java8", "health", "pipeline"}


def run_taurus(config: str = "api", mode: str = "standard") -> dict[str, Any]:
    """Uruchamia test Taurus przez run-taurus.ps1."""
    if config not in TAURUS_CONFIGS:
        return {"error": f"Nieznana konfiguracja: {config}"}
    if mode not in ALLOWED_MODES:
        return {"error": f"Niedozwolony tryb: {mode}"}
    cfg_file = TAURUS_CONFIGS[config]
